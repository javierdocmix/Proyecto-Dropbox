			...
			public void onClick(View v) {
				...
				if (email == null || email.length() < 1 || pswd == null || pswd.length() < 1) {
					...
					Toast toast = Toast.makeText(context,
							"Introduzca correo y contrase�a", duration);
					toast.show();

				} else {

					// Comprobamos que el correo y contrase�a aportados pertenecen a una cuenta de dropbox
					
					
					
					// Creamos el Intent
					Intent intent = new Intent(Dropbox1Activity.this, Logueado.class);
					// A�adimos la informaci�n correo al intent que se le pasa a la nueva actividad
					Bundle b = new Bundle();
